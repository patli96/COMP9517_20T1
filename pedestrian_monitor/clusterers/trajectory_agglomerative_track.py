...
def compute(  # This function will be called with named parameters, so please do not change the parameter name
    ...
    if 'group_counter' not in storage:
        storage['group_counter'] = 0
    ...
    for cl, ps in clusters.items():
        for pg, pps in previous_group_records[0].items():
            for p in ps:
                if p in pps:
                    groups[pg] = ps
                    entering_members[pg] = [x for x in ps if x not in pps]
                    used_clusters.add(cl)
                    break
        if cl not in used_clusters:
            entering_members[storage['group_counter']] = ps
            groups[storage['group_counter']] = ps
            storage['group_counter'] += 1

    for g, ps in previous_group_records[0].items():
        leaving_members[g] = []
        for p in ps:
            if g not in groups or p not in groups[g]:
                leaving_members[g].append(p)
